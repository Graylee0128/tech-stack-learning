...
### 你可以把五種常見 I/O model 記成這張表

| 模型 | 等資料時 | 資料就緒後 | 直覺特徵 |
|------|------|------|------|
| blocking I/O | 呼叫執行緒被卡住 | 同一個呼叫把資料搬回來 | 最直覺，但很佔 thread |
| non-blocking I/O | 不會卡住，自己反覆檢查 | 你自己再呼叫讀取 | 容易變 busy polling |
| I/O multiplexing | 卡在 `select/poll/epoll` 等事件 | 事件來了後仍要自己 `read()` | 最常見的高併發主流 |
| signal-driven I/O | 先註冊訊號 | 收到訊號後再自己讀 | 理論上可行，實務上不常是主流 |
| asynchronous I/O | 整件事丟給系統 | 系統做完再通知 | completion 模式，不只通知 ready |
